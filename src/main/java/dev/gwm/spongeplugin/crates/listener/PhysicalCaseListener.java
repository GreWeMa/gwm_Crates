...
public final class PhysicalCaseListener {
    ...
    public PhysicalCaseListener(Language language) {
        this.language = language;
    }

    @Listener
    public void cancelOffhandEvent(InteractBlockEvent.Primary.OffHand event, @First Player player) {
        event.getTargetBlock().getLocation().flatMap(this::findManager).
    ...
    public void openBlockCase(InteractBlockEvent.Secondary.MainHand event, @First Player player) {
        ...
        event.getTargetBlock().getLocation().flatMap(this::findManager).ifPresent(manager -> {
    ...
    private Optional<Manager> findManager(Location<World> location) {
